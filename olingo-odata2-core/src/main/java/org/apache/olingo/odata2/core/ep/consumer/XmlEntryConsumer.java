...
public class XmlEntryConsumer {
  ...
  private void doCallback(final EntityProviderReadProperties readProperties,
    ...
    try {
      if (isFeed) {
        ReadFeedResult callbackInfo = new ReadFeedResult(readProperties, navigationProperty, (ODataFeed) content, entryMetadata.getId());
        callback.handleReadFeed(callbackInfo);
      } else {
        ReadEntryResult callbackInfo = new ReadEntryResult(readProperties, navigationProperty, (ODataEntry) content, entryMetadata.getId());
